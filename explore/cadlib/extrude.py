...
class CoordSystem(object):
    ...
    def numericalize(self, n=256):
        ...
        self.origin = (
            ((self.origin + 1.0) / 2 * n).round().clip(min=0, max=n - 1).astype(int)
        )
        tmp = np.array([self._theta, self._phi, self._gamma])
        self._theta, self._phi, self._gamma = (
            ((tmp / np.pi + 1.0) / 2 * n).round().clip(min=0, max=n - 1).astype(int)
        )
...
class Extrude(object):
    ...
    def from_dict(all_stat, extrude_id, sketch_dim=256):
        ...
        for i in range(len(extrude_entity["profiles"])):
            sket_id, profile_id = (
                extrude_entity["profiles"][i]["sketch"],
                extrude_entity["profiles"][i]["profile"],
            )
            ...
            sket_pos = (
                point[0] * sket_plane.x_axis
                + point[1] * sket_plane.y_axis
                + sket_plane.origin
            )
    ...
    def __str__(self):
        ...
        s += "\n  -operation:{}, type:{}, extent_one:{}, extent_two:{}".format(
            self.operation,
            self.extent_type,
            self.extent_one.round(4),
            self.extent_two.round(4),
        )
        s += "\n  -" + str(self.profile)
        return s
    ...
    def numericalize(self, n=256):
        ...
        self.extent_one = (
            ((self.extent_one + 1.0) / 2 * n).round().clip(min=0, max=n - 1).astype(int)
        )
        self.extent_two = (
            ((self.extent_two + 1.0) / 2 * n).round().clip(min=0, max=n - 1).astype(int)
        )
        self.operation = int(self.operation)
        self.extent_type = int(self.extent_type)

        self.sketch_pos = (
            ((self.sketch_pos + 1.0) / 2 * n).round().clip(min=0, max=n - 1).astype(int)
        )
        self.sketch_size = (
            (self.sketch_size / 2 * n).round().clip(min=0, max=n - 1).astype(int)
        )
...
class CADSequence(object):
    ...
    def __str__(self):
        return "" + "\n".join(
            ["({})".format(i) + str(ext) for i, ext in enumerate(self.seq)]
        )

    def to_vector(
        self, max_n_ext=10, max_n_loops=6, max_len_loop=15, max_total_len=60, pad=False
    ):
    ...
    def random_transform(self):
        for item in self.seq:
            ...
            translate = (
                np.array([random.randint(-5, 5), random.randint(-5, 5)], dtype=int)
                + 128
            )
